...
--FROM employees; 오류 : 날짜끼리의 데이터에 +는 연산되지 않는다 (날짜1개에다 숫자1개는 상관없음)
...
FROM EMPLOYEES 
WHERE salary >= 10000	--select절보다 where절이 먼저 실행되니까 as명을 쓰면 안됨
ORDER BY SALARY;
...
-- 6) SQL연산자
-- BETWEEN a AND b : a 이상 b이하인 조건 => 범위내에 포함하면 참

-- employees 테이블에서 salary가 10000 이상 12000이하인 직원의 
--first_name, last_name, salary를 salary 오름차순으로 조회
SELECT first_name, last_name, salary 급여
FROM EMPLOYEES
WHERE salary BETWEEN 10000 AND 12000;	--별칭이 만들어지기전에는 급여를 쓰지못하니까 '급여'사용 불가능
...
WHERE salary IN(10000, 11000, 12000);
 ...
 -- % : ~아무거나 _ : 자리수
	...
	FROM EMPLOYEES
	-- WHERE COMMISSION_PCT = NULL; 이렇게 하면 데이터가 안나온다 NULL값을 확인할 때는 =(관계연산자)가 아닌 IS NULL로 확인한다
