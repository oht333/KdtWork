...
SELECT EMPLOYEE_ID, FIRST_NAME, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEES e2 WHERE e2.DEPARTMENT_ID = e.DEPARTMENT_ID)
FROM EMPLOYEES e;


--FROM절 서브쿼리
-- 부서별 평균급여를 계산한 후, 평균 급여보다 높은 직원 초회하기
SELECT E.EMPLOYEE_ID, E.FIRST_NAME , E.SALARY, T.AVG_SALARY
FROM EMPLOYEES E 
	JOIN (SELECT DEPARTMENT_IDK AVG(SALARY) AVG_SALARY 
		FROM EMPLOYEES
...
SELECT AVG(SALARY) FROM (SEL;


-- [실습] 

--평균 급여보다 낮은 사원드의 급여를 20%(1.2) 연상한 결과 테이블을 조회하라
